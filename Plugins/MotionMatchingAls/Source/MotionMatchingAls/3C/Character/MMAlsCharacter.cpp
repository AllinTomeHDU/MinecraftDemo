...
#pragma region HitReacting
void AMMAlsCharacter::SetHitReacting(const bool bHitReacting)
{
	bool bPrevHitReactiong = bIsHitReacting;
	bIsHitReacting = bHitReacting;
	OnHitReactingChanged(bPrevHitReactiong);
	if (!HasAuthority())
	{
		Server_SetHitReacting(bHitReacting);
	}
}

void AMMAlsCharacter::Server_SetHitReacting_Implementation(const bool bHitReacting)
{
	bool bPrevHitReactiong = bIsHitReacting;
	bIsHitReacting = bHitReacting;
	OnHitReactingChanged(bPrevHitReactiong);
...
void AMMAlsCharacter::OnRep_HitReacting(bool bPrevHitReactiong)
{
	OnHitReactingChanged(bPrevHitReactiong);
}

void AMMAlsCharacter::OnHitReactingChanged(bool bPrevHitReactiong)
{
	if (auto MoveComp = Cast<UMMAlsMovementComponent>(GetCharacterMovement()))
	{
		MoveComp->SetMaxSpeeds();
	}
}
#pragma endregion
