...
{
	CHECK_CHARACTER_AND_MOVECOMP;
	if (Chr->IsHitReacting() || MoveComp->IsAiming()) return;
...
{
	CHECK_CHARACTER_AND_MOVECOMP;
	if (Chr->IsHitReacting() || MoveComp->IsCrouching() || MoveComp->IsFalling()) return;
...
{
	CHECK_CHARACTER_AND_MOVECOMP;
	if (Chr->IsHitReacting() || MoveComp->IsAiming() || MoveComp->GetGait() == EMMAlsGait::Sprinting) return;
