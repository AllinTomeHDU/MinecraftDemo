...
class SESSIONSSYSTEM_API UMultiplayerSessionsSubsystem : public UGameInstanceSubsystem
{
	...
	// 默认配置，单局游戏最大可连接数，用于支持动态修改最大连接数
	// 这样CurrentPlayerNum和MaxPlayerNum完全由自定义参数控制
	UPROPERTY(BlueprintReadOnly, meta = (AllowPrivateAccess = "true"))
	int32 MaxClientConnections{ 5 };
...
private:
	...
	FSessionsCreateParams SessionCreateParams;

public:
	FORCEINLINE const FSessionsCreateParams& GetSessionCreateParams() const { return SessionCreateParams; }
...
private:
	...
	EGameEnterMode GameEnterMode = EGameEnterMode::Startup;

public:
	FORCEINLINE EGameEnterMode GetGameEnterMode() const { return GameEnterMode; }
	FORCEINLINE void SetGameEnterMode(EGameEnterMode InGameEnterMode) { GameEnterMode = InGameEnterMode; }
